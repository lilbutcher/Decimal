...
s21_decimal s21_decimal_set_bit(s21_decimal decimal, int index) {
  decimal.bits[index / MAX_BITS_INT] =
      s21_set_bit(decimal.bits[index / MAX_BITS_INT], index % MAX_BITS_INT);
  return decimal;
}
...
/*
  returns an index of the first occurance of a non-zero bit starting from the
  highest if there are no non-zero bits, returns -1
*/
...
s21_decimal s21_decimal_binary_shift_right_one(s21_decimal decimal) {
  ...
  for (size_t i = 3; i > 0; --i) {
    ...
    if (lowest)
      res.bits[i - 1] = s21_set_bit(res.bits[i - 1], MAX_BITS_INT - 1);
  }
  return res;
}
...
/*
  shifts the s21_big_decimal by *shift* bits to the left (s21_big_decimal <<
  shift)
*/
...
/*
  shifts the s21_big_decimal by *shift* bits to the right (s21_big_decimal >>
  shift)
*/
s21_big_decimal s21_big_decimal_binary_shift_right(s21_big_decimal decimal,
  ...
  while (shift > 0) {
    ...
    if (overflow)
      res.decimals[0] = s21_decimal_set_bit(res.decimals[0], MAX_BITS - 1);
    --shift;
  }
  return res;
}
...
/*
  ...
  returns -1 if value_1 < value_2, 0 if value_1 == value_2, 1 if value_1 >
  value_2
*/
...
int s21_big_decimal_binary_compare(s21_big_decimal value_1,
  ...
  int compare_res =
      s21_decimal_binary_compare(value_1.decimals[1], value_2.decimals[1]);
  if (!compare_res) {
    compare_res =
        s21_decimal_binary_compare(value_1.decimals[0], value_2.decimals[0]);
  }
  return compare_res;
}
...
s21_big_decimal s21_big_decimal_binary_addition(s21_big_decimal value_1,
  ...
  while (!s21_decimal_binary_is_zero(tmp.decimals[0]) ||
    ...
    overflow.decimals[0] =
        s21_decimal_binary_and(res.decimals[0], tmp.decimals[0]);
    overflow.decimals[1] =
        s21_decimal_binary_and(res.decimals[1], tmp.decimals[1]);
...
/*
  binary multiplication using bitwise operations
  returns s21_big_decimal - product of s21_big_decimal parameter and s21_decimal
  parameter
*/
...
s21_decimal s21_decimal_binary_division(s21_decimal dividend,
  ...
  if (s21_decimal_binary_is_zero(dividend)) {
    ...
    while (shift >= 0) {
      if (subtraction) {
        partial_remainder =
            s21_decimal_binary_subtraction(dividend_tmp, shifted_divisor);
      } else {
        partial_remainder =
            s21_decimal_binary_addition(dividend_tmp, shifted_divisor);
      }
    ...
    partial_remainder = s21_decimal_binary_shift_right(
        partial_remainder, non_zero_dividend - non_zero_divisor);
  }
  res = quotient;
  if (remainder != NULL) {
...
/*
  ...
  returns s21_big_decimal (uint256) - quotient of the two s21_big_decimal
  parameters (if you don't need a remainder — pass NULL as the third arg)
*/
s21_big_decimal s21_big_decimal_binary_division(s21_big_decimal dividend,
  ...
  if (s21_decimal_binary_is_zero(dividend.decimals[0]) &&
    ...
    s21_big_decimal shifted_divisor =
        s21_big_decimal_binary_shift_left(divisor, shift);
    s21_big_decimal dividend_tmp = dividend;
    s21_comparison_res subtraction = S21_TRUE;
    while (shift >= 0) {
      if (subtraction) {
        partial_remainder =
            s21_big_decimal_binary_subtraction(dividend_tmp, shifted_divisor);
      } else {
        partial_remainder =
            s21_big_decimal_binary_addition(dividend_tmp, shifted_divisor);
      }
    ...
    partial_remainder = s21_big_decimal_binary_shift_right(
        partial_remainder, non_zero_dividend - non_zero_divisor);
  }
  res = quotient;
  if (remainder != NULL) {
...
void s21_set_scale(s21_decimal *dst, int scale) {
  int sign = 0;
  sign = s21_decimal_get_sign(*dst);
  dst->bits[3] = 0;
  scale <<= 16;
  dst->bits[3] = scale | dst->bits[3];
  if (sign) s21_decimal_set_sign(dst, sign);
}
...
int s21_get_scale(s21_decimal decimal) {
  ...
  for (int i = 96 + 23; i >= 96 + 16; i--) {
    scale <<= 1;
    scale |= s21_get_bit(decimal, i);
  }
  return scale;
}

int s21_get_bit(s21_decimal decimal, int index) {
  int num_int = index / MAX_BITS_INT;
  int num_bit = index % MAX_BITS_INT;
  return (decimal.bits[num_int] & (1u << num_bit)) >> num_bit;
}